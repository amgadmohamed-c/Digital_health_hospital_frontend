...
import { doctorAPI } from "../auth/api"; // ✅ only import what's needed
...
const features = [
  {
    ...
    description: "Review all surgeries scheduled for today and track their progress.",
    icon: Scissors,
  ...
  {
    title: "Appointments",
    description: "View upcoming appointments, manage your schedule and patient visits.",
    icon: CalendarDays,
  ...
  {
    title: "Availability",
    description: "Set your weekly availability and manage appointment time slots.",
    icon: Clock,
  ...
  },
  {
    ...
    description: "Update your bio, specialization, and professional information.",
    icon: User,
    link: "/doctor/profile",
...
const coreValues = [
  { icon: ShieldCheck, label: "Safety",     color: "text-violet-700",  ring: "#7c3aed" },
  { icon: Zap,         label: "Precision",  color: "text-blue-700",    ring: "#1d4ed8" },
  { icon: Heart,       label: "Empathy",    color: "text-rose-600",    ring: "#e11d48" },
  { icon: Link2,       label: "Trust",      color: "text-purple-700",  ring: "#6d28d9" },
  { icon: BarChart3,   label: "Excellence", color: "text-fuchsia-700", ring: "#a21caf" },
];
...
export default function DoctorDashboard() {
  ...
  // ✅ Fetch doctor name using the correct doctor endpoint: GET /doctor/me
  useEffect(() => {
    doctorAPI.getMe()
      .then((res) => {
        // Backend returns: { name, email, phone, age, gender, doctorProfile }
        const name = res.data?.name;
        if (name) setDoctorName(name.split(" ")[0]);
      })
      .catch(() => {
        // silently fail — name stays as "Doctor"
      });
  }, []);
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.to(heroInnerRef.current, {
        y: -90, scale: 0.95, opacity: 0, ease: "none",
        scrollTrigger: { trigger: heroRef.current, start: "top top", end: "bottom top", scrub: 1 },
      });

      gsap.fromTo(featuresRef.current, { opacity: 0, y: 50 }, {
      ...
      gsap.to(".feature-card", {
        ...
        scrollTrigger: { trigger: featuresRef.current, start: "top 80%", once: true, invalidateOnRefresh: true },
      });

      gsap.fromTo(imageRef.current, { opacity: 0, x: -70, scale: 0.94, rotate: -2 }, {
        opacity: 1, x: 0, scale: 1, rotate: 0, duration: 1.4, ease: "power4.out",
        scrollTrigger: { trigger: storyRef.current, start: "top 75%", once: true },
      });
      gsap.fromTo(".story-content", { opacity: 0, x: 70, scale: 0.96 }, {
        opacity: 1, x: 0, scale: 1, duration: 1.4, ease: "power4.out",
        scrollTrigger: { trigger: storyRef.current, start: "top 75%", once: true },
      });
      gsap.to(".story-img", {
        y: 60, ease: "none",
        scrollTrigger: { trigger: storyRef.current, start: "top bottom", end: "bottom top", scrub: 1.2 },
      });

      gsap.fromTo(".core-title", { opacity: 0, y: 36 }, {
        opacity: 1, y: 0, duration: 1, ease: "power3.out",
      ...
      gsap.to(coreCards, {
        ...
        stagger: { amount: 0.5, from: "center" }, ease: "back.out(1.5)",
        scrollTrigger: { trigger: coreRef.current, start: "top 88%", once: true, invalidateOnRefresh: true },
      });
      ...
      gsap.to(".glow-mid", {
        y: -80, ease: "none",
        scrollTrigger: { trigger: featuresRef.current, start: "top bottom", end: "bottom top", scrub: 1.5 },
      });
  ...
  return (
    <>
      ...
      >
        <div className="glow-tl pointer-events-none fixed top-[-8rem] left-[-8rem] w-[40rem] h-[40rem] rounded-full bg-violet-400/20 blur-[110px]" />
        <div className="glow-br pointer-events-none fixed bottom-[-8rem] right-[-8rem] w-[40rem] h-[40rem] rounded-full bg-purple-500/20 blur-[110px]" />
        <div className="glow-mid pointer-events-none absolute top-[110vh] left-1/2 -translate-x-1/2 w-[55rem] h-[32rem] rounded-full bg-fuchsia-400/10 blur-[130px]" />

        <div className="relative z-10 px-4 md:px-8 xl:px-16 pb-40">

          {/* SCENE 1 — HERO */}
          <section ref={heroRef} className="min-h-[82vh] flex items-center pt-16 pb-8">
            <div ref={heroInnerRef} className="w-full max-w-4xl">
              ...
              >
                ...
                <div className="absolute -bottom-10 -right-10 w-52 h-52 rounded-full bg-gradient-to-br from-violet-400/25 to-purple-400/15 blur-2xl pointer-events-none" />
                ...
                <h1 className="hero-title text-5xl md:text-7xl font-black text-slate-900 tracking-tight leading-[1.05] mb-6">
                  ...
                  <span className="inline-block" style={{ animation: "wave 2.2s ease-in-out infinite" }}>
                    👋
                  </span>
                </h1>

                <p className="hero-body text-slate-600 text-xl md:text-2xl max-w-2xl leading-relaxed">
                  Manage your surgeries, appointments, and patient consultations —
                  everything you need to deliver exceptional care in one place.
                </p>
          ...
          <section ref={featuresRef} className="mt-12 mb-8">
            <div className="mb-10 flex items-center gap-4">
              ...
              <span className="text-xs font-semibold tracking-[0.25em] uppercase text-slate-400">Quick Access</span>
              <div className="h-px flex-1 bg-gradient-to-r from-transparent via-slate-300/80 to-transparent" />
            </div>
              ...
              {features.map((feature, index) => {
                ...
                return (
                  <Link
                    ...
                    style={{ background: "rgba(255,255,255,0.46)" }}
                  >
                    <div className={`absolute inset-0 opacity-0 group-hover:opacity-15 bg-gradient-to-br ${feature.gradient} transition-opacity duration-500 rounded-[2rem]`} />
                    <div className={`absolute top-0 left-8 right-8 h-[2px] bg-gradient-to-r ${feature.gradient} rounded-b-full opacity-50 group-hover:opacity-100 transition-opacity duration-300`} />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 p-8 text-white">
                  <span className="text-[10px] font-bold tracking-[0.3em] uppercase text-violet-300 block mb-3">Excellence</span>
                  <h2 className="text-3xl font-black mb-3 leading-tight">Advanced Care<br />Every Day</h2>
                  <p className="text-slate-300 text-sm leading-relaxed max-w-xs">
                    Precision medicine powered by modern technology and human compassion.
                  </p>
                </div>
              </div>
              ...
              <div className="story-content">
                ...
                >
                  <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/90 to-transparent" />
                  <div className="absolute -top-16 -right-16 w-52 h-52 rounded-full bg-gradient-to-br from-violet-400/20 to-purple-400/10 blur-2xl pointer-events-none" />

                  <span className="text-[10px] font-bold tracking-[0.3em] uppercase text-violet-600 block mb-5">Our Mission</span>

                  <h2 className="text-4xl md:text-5xl font-black text-slate-900 leading-[1.1] mb-6">
                    The Platform<br />Behind{" "}
                  ...
                  <p className="text-slate-700 text-lg leading-relaxed mb-4">
                    MediCore gives doctors a unified workspace to manage schedules,
                    monitor patients, and collaborate with surgical teams seamlessly.
                  </p>
                  <p className="text-slate-500 leading-relaxed">
                    From real-time chat to surgery coordination — built to let you
                    focus on what matters most: your patients.
                  </p>

                  <div className="flex flex-wrap gap-3 mt-8">
                    {[["500+", "Doctors"], ["50k+", "Appointments"], ["99.9%", "Uptime"]].map(([val, lbl]) => (
                      <div
                        key={lbl}
          ...
          <section className="mt-36">
            <div className="text-center mb-14">
              <span className="core-title text-[10px] font-bold tracking-[0.35em] uppercase text-slate-400 block mb-4">What We Stand For</span>
              <h2 className="core-title text-4xl md:text-5xl font-black text-slate-900">Our Core Values</h2>
            </div>

            <div ref={coreRef} className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-5">
              {coreValues.map(({ icon: Icon, label, color, ring }) => (
                ...
                >
                  ...
                  <div
                    ...
                    style={{ background: `${ring}18`, boxShadow: `0 4px 16px ${ring}28` }}
                  >
